...
use Pronnect\GpWebPay\Http\Request\ApplePayRequest                as HttpApplePayRequest;
...
use Pronnect\GpWebPay\Http\Request\GooglePayRequest               as HttpGooglePayRequest;
...
if ($action === 'create_apple_pay_order' && hasConfig()) {
    try {
        $cents       = amountCents($_POST['amount']);
        $orderNumber = (int)(trim($_POST['order_number']) ?: time());
        $depositFlag = (int)$_POST['deposit_flag'];
        ...
        $request = new HttpApplePayRequest(
            orderNumber: $orderNumber,
            amount:      $cents,
            currency:    $currency,
            depositFlag: $depositFlag,
        ...
        $pdo->prepare(
            'INSERT INTO payments (type, payment_number, order_number, amount, currency_code, http_redirect_url)
             VALUES (:type, :pn, :on, :amt, :cur, :rurl)'
        )->execute([
            'type' => 'apple_pay_order',
            'pn'   => (string)$orderNumber,
            'on'   => (string)$orderNumber,
            'amt'  => $cents,
            'cur'  => $currency,
...
if ($action === 'create_google_pay_order' && hasConfig()) {
    try {
        $cents       = amountCents($_POST['amount']);
        $orderNumber = (int)(trim($_POST['order_number']) ?: time());
        $depositFlag = (int)$_POST['deposit_flag'];
        ...
        $request = new HttpGooglePayRequest(
            orderNumber: $orderNumber,
            amount:      $cents,
            currency:    $currency,
            depositFlag: $depositFlag,
