    ...
    protected function callWS(
        ...
        try {
            ...
            if (isset($soapFault->detail->serviceException)) {
                $exception = $soapFault->detail->serviceException;
                $serviceException = new ServiceException();
                $serviceException->setMessageId($exception->messageId);
                $serviceException->setPrimaryReturnCode((string)($exception->primaryReturnCode ?? null));
                $serviceException->setSecondaryReturnCode((string)($exception->secondaryReturnCode ?? null));
                $serviceException->setSignature((string)$exception->signature ?: null);
            ...
            if (isset($soapFault->detail->cardOnFilePaymentServiceException)) {
                $cofException = $soapFault->detail->cardOnFilePaymentServiceException;
                $serviceException = new CardOnFilePaymentFaultDetail();
                $serviceException->setMessageId($cofException->messageId);
                $serviceException->setPrimaryReturnCode((string)($cofException->primaryReturnCode ?? null));
                $serviceException->setSecondaryReturnCode((string)($cofException->secondaryReturnCode ?? null));
                $serviceException->setAuthenticationLink($cofException->authenticationLink ?? null);
                $serviceException->setSignature((string)$cofException->signature ?: null);
                $validException = $this->getDigestSigner()->verify(
                    (string)$serviceException->getDigest(),
                    base64_decode((string)$serviceException->getSignature())
                );
                ...
                $soapFault->detail->cardOnFilePaymentServiceException = $serviceException;
            }
